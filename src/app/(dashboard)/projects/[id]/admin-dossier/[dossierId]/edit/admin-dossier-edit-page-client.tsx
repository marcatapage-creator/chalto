...
interface AdminDossierEditPageClientProps {
  project: { id: string; name: string }
  dossier: AdminDossier
}

export function AdminDossierEditPageClient({ project, dossier }: AdminDossierEditPageClientProps) {
  ...
  const handleSubmit = async () => {
    setSubmitting(true)
    try {
      const res = await fetchWithTimeout(`/api/admin-dossiers/${dossier.id}`, {
        method: "PATCH",
    ...
    >
      <div className="shrink-0 border-b px-4 py-3 flex items-center gap-3 bg-popover">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium">Modifier le dossier</p>
